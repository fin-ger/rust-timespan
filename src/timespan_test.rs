...
#[test]
fn duration_test() {
    let ts = Timespan::from_str("13:00:00 - 14:00:00").unwrap();
...
fn difference_test() {
    ...
    let t12 = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
    let t21 = Timespan::from_str("11:00:00 - 12:00:00").unwrap();
    assert!(t1.difference(&t2).unwrap() == t12);
    assert!(t2.difference(&t1).unwrap() == t21);

    let t3 = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
    let t4 = Timespan::from_str("11:00:00 - 12:00:00").unwrap();
    assert!(t3.difference(&t4).unwrap() == t3);
    assert!(t4.difference(&t3).unwrap() == t4);

    let t5 = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
    let t6 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    assert!(t5.difference(&t4).unwrap() == t5);
    assert!(t6.difference(&t3).unwrap() == t6);

    let t7 = Timespan::from_str("09:00:00 - 12:00:00").unwrap();
    let t8 = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
    let t9 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    let ta = Timespan::from_str("11:00:00 - 12:00:00").unwrap();
...
fn symmetric_difference_test() {
    let t1 = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
    let t2 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    let t3 = Timespan::from_str("09:00:00 - 11:00:00").unwrap();
    let t4 = Timespan::from_str("11:00:00 - 12:00:00").unwrap();
...
fn intersection_test() {
    let t1 = Timespan::from_str("09:00:00 - 12:00:00").unwrap();
    let t2 = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
    let t3 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    let t4 = Timespan::from_str("11:00:00 - 12:00:00").unwrap();
    let t5 = Timespan::from_str("12:00:00 - 13:00:00").unwrap();
    let t6 = Timespan::from_str("11:00:00 - 13:00:00").unwrap();
...
fn union_test() {
    ...
    let t4 = Timespan::from_str("12:00:00 - 14:00:00").unwrap();

    let t12 = Timespan::from_str("09:00:00 - 12:00:00").unwrap();
    let t13 = Timespan::from_str("09:00:00 - 13:00:00").unwrap();
...
#[test]
fn contains_test() {
    let ts = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
...
fn is_subset_test() {
    let t1 = Timespan::from_str("09:00:00 - 12:00:00").unwrap();
    let t2 = Timespan::from_str("08:00:00 - 09:00:00").unwrap();
    let t3 = Timespan::from_str("09:00:00 - 10:00:00").unwrap();
    let t4 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    let t5 = Timespan::from_str("11:00:00 - 12:00:00").unwrap();
    let t6 = Timespan::from_str("12:00:00 - 13:00:00").unwrap();
...
#[test]
fn split_off_test() {
    let ts = Timespan::from_str("10:00:00 - 12:00:00").unwrap();
    let t1 = NaiveTime::from_str("09:00:00").unwrap();
    ...
    let ts1 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    let ts2 = Timespan::from_str("11:00:00 - 12:00:00").unwrap();
...
#[test]
fn append_test() {
    let t1 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    let t2 = Timespan::from_str("10:00:00 - 12:00:00").unwrap();
...
fn prepend_test() {
    let t1 = Timespan::from_str("10:00:00 - 11:00:00").unwrap();
    let t2 = Timespan::from_str("09:00:00 - 11:00:00").unwrap();
    let d1 = Duration::hours(1);
    let d2 = Duration::hours(-1);
...
#[test]
fn from_str_test() {
    let parsed = Timespan::from_str("10:45:00 - 15:30:00").unwrap();
    let parsed_reference = Timespan::new(
        NaiveTime::from_hms(10, 45, 0),
        NaiveTime::from_hms(15, 30, 0),
    ).unwrap();
    assert!(parsed == parsed_reference);

    assert!(Timespan::from_str("10.45.00 - 15.30.00").is_err());
    assert!(Timespan::from_str("09:15:00-12:00:00").is_ok());
    assert!(Timespan::from_str("11:11").is_err());
...
mod with_serde {
    ...
    fn serialize_test() {
        let reference =
            TimespanTest { span: Timespan::from_str("09:00:00 - 12:00:00").unwrap() };
    ...
    fn deserialize_test() {
        let reference =
            TimespanTest { span: Timespan::from_str("09:00:00 - 12:00:00").unwrap() };
        let json = r#"{"span":"09:00:00 - 12:00:00"}"#;
        assert!(serde_json::from_str::<TimespanTest>(&json).unwrap() == reference);
    }
}
